...
describe('modules/manager/nix/artifacts', () => {
  beforeEach(() => {
    ...
    // This manager still derives the lock file via getSiblingFileName, and
    // `util/fs` is mocked above, so give it a passthrough implementation.
  ...
  it('includes per-org cross-org trust tokens in --extra-access-tokens', async () => {
    ...
    fs.readLocalFile.mockResolvedValueOnce('new flake.lock');
    hostRules.add({ matchHost: 'github.com', token: 'main-token' });
    hostRules.add({
      hostType: 'github',
  ...
  it('supports docker mode', async () => {
    ...
    expect(execSnapshots).toMatchObject([
      ...
      {
        cmd: `docker run --rm --name=renovate_sidecar --label=renovate_child -v "/tmp/github/some/repo":"/tmp/github/some/repo" -v "/tmp/renovate/cache":"/tmp/renovate/cache" -e CONTAINERBASE_CACHE_DIR -w "/tmp/github/some/repo" ghcr.io/renovatebot/base-image bash -l -c '${updateInputCmdInDocker}'`,
      },
    ]);
  });
