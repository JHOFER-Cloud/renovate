...
- For branch-tracked packages the `rev` and the hash are updated, but a nixpkgs-style `version = "<base>-unstable-YYYY-MM-DD"` string is left as-is — a digest update carries no release timestamp to derive the new date from, and writing a guessed date would be worse than leaving it stale
